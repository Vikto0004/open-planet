...
export async function DELETE(
  ...
  try {
    ...
    const sectionExistsUa = workDirection.ua.sections.find(
      (section: Section) => section.id.toString() === sectionId,
    );
    const sectionExistsEn = workDirection.en.sections.find(
      (section: Section) => section.id.toString() === sectionId,
    );
    ...
    const itemExistsUa = sectionExistsUa.content.find(
      (item: Item) => item.id === budgetCardId,
    );
    const itemExistsEn = sectionExistsEn.content.find(
      (item: Item) => item.id === budgetCardId,
    );
